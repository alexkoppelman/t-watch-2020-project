...
void List::__list_event_cb(lv_obj_t *obj, lv_event_t event)
{
    if (event == LV_EVENT_SHORT_CLICKED) {
...
void List::setListCb(list_event_cb cb)
{
    _cb = cb;
}
...
void MBox::create(const char *text, lv_event_cb_t event_cb, const char **btns, lv_obj_t *par)
{
    if (_mbox != nullptr)return;
...
void MBox::setBtn(const char **btns)
{
    lv_msgbox_add_btns(_mbox, btns);
}
